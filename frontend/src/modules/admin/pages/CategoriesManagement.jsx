  ...
  return (
    <div className="-mx-4 md:-mx-6 lg:-mx-10 -my-4 md:-my-6 lg:-my-10 px-4 md:px-6 lg:px-10 py-4 md:py-6 lg:py-10 bg-[#faf9f6] min-h-screen">
      <div className="space-y-4 md:space-y-5 pb-10">
        {/* Page Header */}
        <div className="flex items-start justify-between gap-4 px-2 md:px-0 pt-4">
          <div className="space-y-1">
            <h1 className="text-xl font-normal font-serif text-[#0d1b2a]">Categories</h1>
            <p className="text-[11px] text-[#718199] mt-0 tracking-tighter font-normal">Manage property verticals, intents, and their sub-categories</p>
          </div>
          <button
          ...
          {Object.keys(groupedCategories).map(vertical => (
            <div key={vertical} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-slate-50 border-b border-slate-200 px-6 py-4">
                <h3 className="text-[12px] uppercase tracking-wider font-bold text-[#0d1b2a]">{vertical}</h3>
              </div>
                ...
                {groupedCategories[vertical].map(cat => (
                  <div key={cat._id} className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h4 className="text-[12px] font-bold text-slate-900">{cat.name}</h4>
                        <span className="inline-block mt-1.5 text-[9px] font-bold px-2 py-0.5 rounded bg-[#c8962a]/10 text-[#c8962a] border border-[#c8962a]/20 uppercase tracking-widest">
                          {cat.intent}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEditModal(cat)} className="p-1.5 text-slate-400 hover:text-[#c8962a] hover:bg-[#c8962a]/10 rounded transition-colors">
                          <Edit2 size={16} />
                        </button>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {cat.subCategories && cat.subCategories.length > 0 ? (
                        cat.subCategories.map((sub, idx) => (
                          <span key={idx} className="px-2.5 py-1 bg-slate-50 text-slate-600 text-[10px] font-bold rounded-full border border-slate-200">
                            {sub}
                          </span>
                        ))
                      ) : (
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-5">
              ...
              <div className="pt-4 border-t border-slate-200 flex justify-end gap-3">
                ...
                <button 
                  ...
                  className="px-5 py-2.5 bg-[#c8962a] text-white text-[11px] font-bold rounded-lg hover:bg-[#B48C35] flex items-center gap-2 disabled:opacity-70 shadow-lg shadow-[#c8962a]/20"
                >
