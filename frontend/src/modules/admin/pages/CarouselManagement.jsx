...
const CarouselManagement = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      localStorage.setItem('homepage_carousel', JSON.stringify(defaultSlides));
    }
  }, []);
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        showToast('Image asset integrated successfully');
      };
  ...
  const handleSave = (e) => {
    ...
    if (editingId) {
      updatedSlides = slides.map(s => s.id === editingId ? { ...formData, id: editingId } : s);
    ...
    localStorage.setItem('homepage_carousel', JSON.stringify(updatedSlides));
    resetForm();
    setActiveTab('list');
  };

  const resetForm = () => {
    setFormData({ title: '', subtitle: '', image: '', badge: '' });
    setEditingId(null);
  };
  ...
  const handleDelete = (id) => {
    ...
    setSlides(updated);
    localStorage.setItem('homepage_carousel', JSON.stringify(updated));
    showToast('Slide removed', 'warning');
  };
          ...
          {activeTab === 'list' ? (
            ...
            >
              {slides.map((slide, index) => (
                <Card key={slide.id} className="p-0 overflow-hidden border-none shadow-soft hover:shadow-xl transition-all duration-500 group">
                     ...
                     <div className="flex-1 p-8 flex flex-col justify-between">
                        <div>
                           <div className="flex items-center justify-between mb-4">
                              <Badge className="bg-primary-50 text-primary-600 border-none font-black text-[9px] uppercase tracking-[0.2em] px-3 py-1.5">{slide.badge}</Badge>
                              <div className="flex items-center gap-1">
                                 ...
                                 <button onClick={() => handleDelete(slide.id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="Remove">
                                    <Trash2 size={18} />
                                 </button>
                              </div>
                           </div>
