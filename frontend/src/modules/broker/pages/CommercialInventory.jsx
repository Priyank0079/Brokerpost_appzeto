...
const CommercialInventory = () => {
  ...
  const [selectedSubType, setSelectedSubType] = useState(() => sessionStorage.getItem(`commercial_${intent}_subtype`) || 'All Sub-types');
  const [isPostModalOpen, setIsPostModalOpen] = useState(() => sessionStorage.getItem('commercial_post_modal_open') === 'true');
  ...
  useEffect(() => {
    sessionStorage.setItem('commercial_post_modal_open', isPostModalOpen);
  }, [isPostModalOpen]);
  ...
  const [selectedPosting, setSelectedPosting] = useState(() => {
    try {
      const saved = sessionStorage.getItem('commercial_edit_posting');
      return saved ? JSON.parse(saved) : null;
    } catch {
      return null;
    }
  });
  const [isEditModalOpen, setIsEditModalOpen] = useState(() => sessionStorage.getItem('commercial_edit_modal_open') === 'true');

  useEffect(() => {
    sessionStorage.setItem('commercial_edit_modal_open', isEditModalOpen);
  }, [isEditModalOpen]);

  useEffect(() => {
    if (selectedPosting) {
      sessionStorage.setItem('commercial_edit_posting', JSON.stringify(selectedPosting));
    } else {
      sessionStorage.removeItem('commercial_edit_posting');
    }
  }, [selectedPosting]);
  ...
  const [searchTerm, setSearchTerm] = useState(() => sessionStorage.getItem(`commercial_${intent}_search`) || urlSearch);
  const [debouncedSearch, setDebouncedSearch] = useState(() => sessionStorage.getItem(`commercial_${intent}_search`) || urlSearch);

  useEffect(() => {
    sessionStorage.setItem(`commercial_${intent}_search`, searchTerm);
  }, [searchTerm, intent]);

  useEffect(() => {
    sessionStorage.setItem(`commercial_${intent}_subtype`, selectedSubType);
  ...
  const renderSubTypeBadge = (subType) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-[10.5px] font-bold tracking-tight border whitespace-nowrap inline-block ${bg}`}>
        {label}
      </span>
    );
  };
